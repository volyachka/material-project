...
def get_predictions(steps):
    ...
    return max(roc_auc_score(y, preds[:, 0]), roc_auc_score(y, preds[:, 1]))
...
if __name__ == "__main__":
    ...
    lenn = len(y)
    NUM_STEPS = 7
    roc_auc = np.zeros(NUM_STEPS)
    for i, steps in enumerate(range(NUM_STEPS)):
        roc_auc[i] =get_predictions(steps)
        np.savetxt('featureselection_results/classes.txt', roc_auc, fmt='%.4e', delimiter=' ', newline='\n')